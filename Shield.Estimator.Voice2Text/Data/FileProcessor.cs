...
public class FileProcessor
{
    ...
    /// FasterWhisper
    /// </summary>
    public async Task ProcessFasterExistingFilesAsync(string inputPath, string outputPath, ProcessState state, CancellationTokenSource? cts = default)
    {
    ...
    /// WhisperNet
    /// </summary>
    public async Task ProcessNetExistingFilesAsync(string inputPath, string outputPath, string selectedModel, ProcessState state, bool UseWordTamestamps = true, CancellationTokenSource? cts = default)
    {
        _cts = cts;
        var mediaExtensions = GetSupportedMediaExtensions();
    ...
    private async Task ProcessQueueAsync(string outputPath, string selectedModel, ProcessState state, bool UseWordTamestamps = true)
    {
        try
        {
            while (!_cts.Token.IsCancellationRequested)
        ...
        {
            UpdateConsole(state, $"Отменено пользователем: {ex.Message}", true);
        }
        catch (Exception ex)
        {
            UpdateConsole(state, $"Исключение => {ex.Message}", true);
        }
    }
    ...
    private async Task ProcessAudioFileAsync(string filePath, string outputPath, string selectedModel, ProcessState state, CancellationToken ct, bool UseWordTamestamps = true)
    {
        try
        {
    ...
    private void UpdateConsole(ProcessState state, string message, bool prepend)
    {
