...
namespace Shield.Estimator.Shared.Components.Modules.AiEstimateDb
{
    public class EFCoreQuery
    {
        ...
        {
            List <SprSpeechTable> tempList = await context.SprSpeechTables
        ...
        public static async Task<(byte[]?, byte[]?, string?, string?)> GetAudioDataAsync(long? key, BaseDbContext context)
        {
            var results = await context.SprSpData1Tables.Where(x => x.SInckey == key).Select(x => new
            {
            ...
            var results2 = await context.SprSpeechTables.Where(x => x.SInckey == key).Select(x => new
            {
