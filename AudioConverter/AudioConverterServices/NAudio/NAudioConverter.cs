...
public class NAudioConverter : IAudioConverter
{
    ...
    {
        var format = new WaveFormat(_targetSampleRate, 16, 1); // Используем настройки класса
        var leftFile = await CreateTempWavFileAsync(audioDataLeft, format);
        var rightFile = audioDataRight != null ? await CreateTempWavFileAsync(audioDataRight, format) : null;
    ...
    public async Task<(int Duration, byte[] Left, byte[] Right)> ConvertFileToByteArrayAsync(string inputFileName)
    {
        ...
        using var fileStream = new FileStream(inputFileName, FileMode.Open, FileAccess.Read, FileShare.Read);
        var sampleProvider = CreateSampleProvider(inputFileName, fileStream);

        var resampledProvider = sampleProvider.WaveFormat.SampleRate != _targetSampleRate
            ? new WdlResamplingSampleProvider(sampleProvider, _targetSampleRate)
            : sampleProvider;

        await using var outputStream = new MemoryStream();
        WaveFileWriter.WriteWavFileToStream(outputStream, resampledProvider.ToWaveProvider16());
        ...
        {
            1 => (duration, buffer, null),
            2 => (duration, GetLeftChannel(buffer), GetRightChannel(buffer)),
            _ => throw new NotSupportedException($"Unsupported number of channels: {channels}")
        };
    }

    private byte[] GetLeftChannel(byte[] interleaved)
    {
        var result = new byte[interleaved.Length / 2];
        for (int i = 0; i < result.Length; i += 2)
        {
            result[i] = interleaved[i * 2];
            result[i + 1] = interleaved[i * 2 + 1];
        }
        return result;
    }

    private byte[] GetRightChannel(byte[] interleaved)
    {
        var result = new byte[interleaved.Length / 2];
        for (int i = 0; i < result.Length; i += 2)
        {
            result[i] = interleaved[i * 2 + 2];
            result[i + 1] = interleaved[i * 2 + 3];
        }
        return result;
    }
    ...
    private async Task<string> CreateTempWavFileAsync(byte[] pcmData, WaveFormat format)
    {
        if (!Directory.Exists(_tempDirectory))
            Directory.CreateDirectory(_tempDirectory);
        ...
        await using (var writer = new WaveFileWriter(tempFile, format))
        {
            await writer.WriteAsync(pcmData, 0, pcmData.Length);
        }
        return tempFile;
